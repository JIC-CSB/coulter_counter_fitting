...
for (i in 1:length(files)) {
  data <- read.coulter.data(files[i])
  plot <- ggplot(data, aes(x=bin.diameter, y=percvol.density)) +
    geom_line() + ggtitle(basenames[i])
  fname <- paste(tools::file_path_sans_ext(basename(basenames[i])), '.pdf', sep='')
  ggsave(fname, plot)
}

write_csv2(fittings, "all_fittings.csv")
